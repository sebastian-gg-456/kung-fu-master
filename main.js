...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        const playerSize = 155;
        const playerHeight = height - 50 - (playerSize / 2); // Ajusta el centro según el tamaño
        ...
        // Background
        this.add.rectangle(width / 2, height / 2, width, height, 0x87CEEB); // Sky blue
        // If background image exists: this.add.image(width / 2, height / 2, 'background');
        ...
        // Player
        this.player = this.physics.add.sprite(100, playerHeight, 'player'); // Positioned above ground, higher up
        this.player.setDisplaySize(155, 155);
        this.player.body.setSize(155, 155);
    ...
    update() {
        ...
        if (attackPressed) {
            ...
            } else if (this.isAttacking && this.comboCount === 1 && (currentTime - this.lastAttackTime > 150)) {
                // Second punch (combo)
                this.comboCount = 2;
                this.player.anims.play('punch2', true);
                this.lastAttackTime = currentTime;
            } else if (this.isAttacking && this.comboCount === 2 && (currentTime - this.lastAttackTime > 150)) {
                // Continue combo alternating
                this.player.anims.play('punchCombo', true);
                this.lastAttackTime = currentTime;
            }
        }

        // Reset attack after quick timeout
        if (this.isAttacking && (currentTime - this.lastAttackTime > 300)) {
            this.isAttacking = false;
            this.comboCount = 0;
    ...
    spawnEnemy() {
        const height = this.scale.height;
        const width = this.scale.width;
        const playerHeight = height - 250;
        const enemy = this.physics.add.sprite(width + 50, playerHeight, null);
